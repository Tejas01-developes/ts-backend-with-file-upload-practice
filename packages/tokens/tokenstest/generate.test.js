...
describe("token generation test", () => {
    ...
    it("generate access token", async () => {
        jwt.sign.mockReturnValue("access token string");
        const res = accesstoken("12345");
        expect(res).toBe("access token string");
        expect(jwt.sign).toHaveBeenCalledWith({ id: "12345" }, "gsrfgbhsdrgsdrgbstg", { expiresIn: "15m" });
    });
    it("generate refresh token", async () => {
        jwt.sign.mockReturnValue("refresh token string");
        const res = refreshtoken("12345");
        expect(res).toBe("refresh token string");
        expect(jwt.sign).toHaveBeenCalledWith({ id: "12345" }, "gibtigbntgibnrginr", { expiresIn: "7d" });
    });
});
